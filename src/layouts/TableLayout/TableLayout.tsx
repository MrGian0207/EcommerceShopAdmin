              ...
              <tr key={item._id}>
                      ...
                      <img
                        ...
                        src={
                          item.image || item.imageDefault
                            ? item.image || item.imageDefault
                            : variantArray[index]?.variantImagesFile?.[0] ||
                              images.userDefaults
                        }
                      />
                      <h4>
                ...
                {createdAt && (
                  <td className={cx('created-at')}>
                    {format(new Date(item.createdAt as string), 'dd MMM yyyy')}
                  </td>
                )}
                ...
                {status && (
                  <td className={cx('status')}>
                    {quantityArray && quantityArray.length > 0 && (
                      <StatusItems quantity={quantityArray[index]} />
                    )}
                    {item?.statusDelivery && (
                    ...
                    <div className={cx('feature-product')}>
                      <div className={cx('toggle-box')}>
                        <input
                          onChange={(e) => {
                            e.target.checked =
                              featureArray[index]?.feature === 'active'
                                ? true
                                : false;
                          }}
                          checked={
                        ...
                        <label
                          onClick={(e) => {
                            ...
                            setFeatureArray((featureArray) => {
                              featureArray.forEach((value, newIndex) => {
                                if (newIndex === index) {
                                  featureArray[index].feature === 'active'
                                    ? (featureArray[newIndex].feature =
                                        'inactive')
                                    : (featureArray[newIndex].feature =
                                        'active');
                                }
                            ...
                            setFeatureUpdates(featureArray[index]);
                          }}
                ...
                {actions && (
                  <td>
                    <div className={cx('actions')}>
                      ...
                      {/* Chỉnh sửa sản phẩm  */}
                      {editButton && (
                        <Button to={`${path}/${item._id}`} className="edit-btn">
                      ...
                      {deleteButton && (
                        <Button
                          onClick={() => {
                            if (!deleteButtonOnclick && handleDeteleToastify) {
                              handleDeteleToastify(
                                item?.name as string,
                                item?._id as string,
                                path,
