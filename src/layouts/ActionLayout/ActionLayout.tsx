  ...
  const handleFormSubmitWithVariant: FormEventHandler<HTMLFormElement> = (
    ...
    if (VariantArray && VariantArray.length > 0) {
      ...
      VariantArray.forEach((variant) => {
        ...
        formData.append('variantQuantity', variant.variantQuantity as string);
        formData.append(
          'variantRegularPrice',
          variant.variantRegularPrice as string,
        );
        formData.append('variantSalePrice', variant.variantSalePrice as string);
  ...
  return (
    <>
      <form
        ...
        onSubmit={VariantArray ? handleFormSubmitWithVariant : handleFormSubmit}
      >
        <div className={cx('left-column')}>{leftColumn}</div>
        <div className={cx('right-column')}>
          {rightColumn}
          <button ref={submit_ButtonRef} type="submit" className={cx('button')}>
